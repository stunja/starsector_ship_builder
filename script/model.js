  ...
  fetchAndInjectAdditionalWeaponProperties: async function () {
    ...
    const processWeapon = async (weaponObj) => {
      try {
        ...
        const cleanData = cleanWeaponData(dirtyData);
...
const genericHelperFunction = {
  ...
  overwriteWeaponTypeWithForcedOverwrite: async function () {
    try {
      ...
      console.log(
        newArray.filter(
          (weapon) => weapon.additionalWeaponData.mountTypeOverride
        )
      );
    } catch (err) {
...
// const dModsAndBuildIn = allShipHulls.filter((hullMod) =>
//   hullMod.hidden === "TRUE" ? hullMod : ""
// );

//! I need to clear old functions, and only edit data I intend to actually use.
// const convertTagsFromStringIntoAndArray = function () {
//   state.allShipHulls.forEach((hullmod) => {
//     if (hullmod.tags !== "" && hullmod.tags) {
//       hullmod.tagsArray = Object.values(hullmod.tags)
//         .join("")
//         .replace(/[""]/g, "")
//         .replace(/\s+/g, "")
//         .split("|");
//     }
//     if (!hullmod.tags) {
//       hullmod.tagsArray = ["UNAVAILABLE"];
//     }
//   });
// };
// armoredWeaponMounts() {
//   console.log("armoredWeaponMounts");
//   const { _baseArmor, currentArmor } = state.currentShipBuild;
//   let target = `currentArmor`;
//   // Increases the durability of all weapons by 100% and reduces recoil by 25%,
//   // decreases weapon turn rate by 25%
//   // Also increases the ship's armor by 10%.
//   // state.currentShipBuild[target] += _baseArmor * 0.1;
//   state.currentShipBuild[target] += _baseArmor * 0.1;
//   return target;
// },
//! Unused properties dont know
// c_f: ""
// ​
// c_s: ""
// ​
...
//   console.log("additionalBerthing");
//   const { currentShipBuild } = state;
//   // const { shipSize } = currentShipBuild;
//   const maxCrewCalc = () => {
//     const targetProperty = "currentMaxCrew";
//     const baseProperty = "_baseMaxCrew";

//     const valueByShipSize = hullModsHolderhullModGenericBasedOnHullSizeCalc(
//       state.currentShipBuild.shipSize,
//       200,
//       100,
//       60,
...
//     const percentValue = 0.3; // 30% of base value
//     const valueByPercentFromBase =
//       currentShipBuild[baseProperty] * percentValue;
...
//         : valueByPercentFromBase;
//     //
//     state.currentShipBuild[targetProperty] += finalValue;
//     //
//     return targetProperty;
//   };

//   const supplyIncreaseIfCivilianShip = () => {
//     if (civilianHullModCheck() === false) return;
//     const targetProperty = "currentSuppliesPerMonth";
//     const percentValue = 2;
//     //
//     state.currentShipBuild[targetProperty] *= percentValue;
//     return targetProperty;
//   };
//   return [maxCrewCalc(), supplyIncreaseIfCivilianShip()]; //
// },
// const additionalHullModsTagInjection = function (data) {
//! unused
// data.forEach((hullMod) => console.warn(hullMod.tagsArrayToUse));
// data.forEach((hullMod) => console.log(hullMod.name));
// hullModTagInjection.forEach((e) => console.log(e));
// const name = "Accelerated Shields";
// const injectGroup = Object.entries(hullModTagInjection);
// // const extractedValue = injectGroup.map((e) => e.includes(name));
// const extractedValue = tagArray.map((e) => {
//   if (e[1].includes(name)) return e;
// });
// console.log(extractedValue);
// };
// //
// const { currentShipBuild } = state;
// const targetProperty = "currentArmor";
// const baseProperty = "_baseArmor";
// const percentValue = 0.1;
// const finalValue = currentShipBuild[baseProperty] * percentValue;
// //
// state.currentShipBuild[targetProperty] += finalValue;
// return [targetProperty];
...
//   state.currentShipBuild = { hullMods: { buildInHullMods: buildInHullMods } };
// };
// const civilianHullModCheck = function () {
//   const { buildInHullMods } = state.currentShipBuild.hullMods;
//   const targetId = "civgrade";
//   const isThisShipCiv = buildInHullMods.map((e) => e.id === targetId);
//   return isThisShipCiv;
// };

// fetchAndInjectAdditionalWeaponProperties: async function () {
// 	state.allWeapons = await Promise.all(
// 		state.allWeapons.map(async (weaponObj) => {
// 			try {
// 				const res = await fetch(
// 					`./starsectorData/weapons/${weaponObj.id}.wpn`
// 				);
// 				if (!res.ok) {
// 					throw new Error(`HTTP error! status: ${res.status}`);
// 				}
// 				const dirtyData = await res.text();
...
// 							.split(",")
// 							.map((item) => {
// 								// Trim spaces and check if the item is not a number
// 								const trimmedItem = item.trim();
// 								return isNaN(trimmedItem) ? `"${trimmedItem}"` : trimmedItem;
// 							})
// 							.join(",")}]`;
...
// 				const additionalWeaponData = whatKeysToInject.reduce(
// 					(acc, weapon) => {
// 						acc[weapon] = weaponDataFinal[weapon];
// 						return acc;
// 					},
// 					{}
// 				);

// 				const finalArray = { ...weaponObj, ...{ additionalWeaponData } };
// 				return finalArray;
// 			} catch (err) {
